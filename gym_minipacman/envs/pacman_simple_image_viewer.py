    ...
    def imshow(self, arr):
        ...
        if self.window is None:
            self.window = pyglet.window.Window(
                width=self.scale*width*4, height=self.scale*height*4, display=self.display, vsync=False, resizable=True)
            self.width = width
            self.height = height
            self.isopen = True
        ...
        resized_image = np.zeros(
            (height*self.scale, width*self.scale, _channels)).astype('uint8')
        for y in range(height):
            for x in range(width):
                for ys in range(self.scale):
                    for xs in range(self.scale):
                        resized_image[y*self.scale + ys][x *
                                                         self.scale + xs] = arr[y][x]
        arr = resized_image
